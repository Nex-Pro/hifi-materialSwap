...
(function(){
    ...
    var FlowDebug = function () {
        ...
        this.setDebugCube = function (cubeName, cubePosition, cubeRotation, cubeDimensions, shapeColor, forceRendering) {
            ...
            if (!doRender) return;
            var position = cubePosition ? cubePosition : {
                x: 0,
                y: 0,
                z: 0
            };
            var rotation = cubeRotation ? cubeRotation : {
                x: 0,
                y: 0,
                z: 0,
                w: 0
            };
            var dimensions = cubeDimensions ? cubeDimensions : {
                x: 1,
                y: 1,
                z: 1
            };
            var color = shapeColor !== undefined ? shapeColor : {
                red: 0,
                green: 255,
                blue: 255
            };
        ...
        this.setDebugLine = function (lineName, startPosition, endPosition, shapeColor, forceRendering) {
            ...
            var end = endPosition ? endPosition : {
                x: 0,
                y: 1,
                z: 0
            };
            var color = shapeColor ? shapeColor : {
                red: 0,
                green: 255,
                blue: 255
            };
            ...
            nearbyAvatars.forEach(function (avatarID) {
                ...
                if (avatarIndex === -1) {
                    ...
                    for (i = 0; i < HAND_COLLISION_JOINTS.length; i++) {
                        ...
                        var handCollisionSettings = {
                            type: "sphere",
                            radius: HAND_COLLISION_RADIUS,
                            offset: {
                                x: 0,
                                y: 0,
                                z: 0
    ...
    var FlowCollisionSystem = function () {
                    ...
                    case "offset":
                        {
                            ...
                            self.collisionSpheres[collisionIndex].initialOffset = {
                                x: offset.x,
                                y: value,
                                z: offset.z
                            };
        ...
        this.getCollisionData = function () {
            ...
            for (var i = 0; i < self.collisionSpheres.length; i++) {
                var data = {
                    type: "sphere",
                    radius: self.collisionSpheres[i].initialRadius,
                    offset: self.collisionSpheres[i].initialOffset
                };
    ...
    var FlowCollisionData = function (offset, position, radius, normal, distance) {
        ...
        this.normal = normal !== undefined ? normal : {
            x: 0,
            y: 0,
            z: 0
        };
    ...
    var FlowCollisionSphere = function (name, jointIndex, jointName, settings) {
        ...
        this.update = function () {
            ...
            collisionDebug.setDebugSphere(self.name, self.position, 2 * self.radius, {
                red: 200,
                green: 10,
                blue: 50
            });
            if (self.attenuation && self.attenuation > 0) {
                collisionDebug.setDebugSphere(self.name + "_att", self.position, 2 * (self.radius + self.attenuation), {
                    red: 120,
                    green: 200,
                    blue: 50
                });
    ...
    var FlowCollisionCube = function (name, jointIndex, settings) {
        ...
        this.update = function () {
            ...
            collisionDebug.setDebugCube(self.name, self.position, self.rotation, self.dimensions, {
                red: 200,
                green: 10,
                blue: 50
            });
        ...
        this.checkCollision = function (point, radius) {
            ...
            if (offsets.x > 0 && offsets.y > 0 && offsets.z > 0) {
                ...
                switch (offsetOrder[0].coordinate) {
                    case "x":
                        normal = {
                            x: centerToJoint.x > 0 ? 1 : -1,
                            y: 0,
                            z: 0
                        };
                        radius = self.dimensions.x / 2;
                        position = VEC3.sum(self.offset, {
                            x: 0,
                            y: centerToJoint.y,
                            z: centerToJoint.z
                        });
    ...
    var getCamelCaseSplit = function (name) {
        var sub = name.replace(/(.)([A-Z][a-z]+)/g, '$1_$2');
        var result = sub.replace(/([a-z0-9])([A-Z])/g, '$1_$2');
        return result.replace(/([A-Za-z])([0-9]+)/g, '$1_$2');
    };
    ...
    var calculateConstraints = function () {
        ...
        flowSkeleton = MyAvatar.getSkeleton().filter(
            function (jointInfo) {
                ...
                var namesplit = getCamelCaseSplit(name).split("_");

                var isFlowJoint = (namesplit.length > 2 &&
                    (namesplit[0].toUpperCase() === FLOW_JOINT_PREFIX.toUpperCase() ||
                     namesplit[0].toUpperCase() === SIM_JOINT_PREFIX.toUpperCase()));

                console.log(namesplit, isFlowJoint);
                if (isFlowJoint) {
                    var group;
